...
The current testing builds are Fedora kernels from the upstream stable kernel 7.0.10, build [kernel-ark](https://gitlab.com/cki-project/kernel-ark)-like by [arkify](https://gitlab.com/knurd42/linux/-/tree/arkify-arkify) to be close to the Fedora kernel.

The kernels are patched with [PATCH v2 3/3 drm/amd/display: Check for flip completion in OTG interrupt handlers](https://lore.kernel.org/amd-gfx/20260519220529.202096-1-sunpeng.li@amd.com/T/#m5fc5bc183f8264232b782dc5637e3b18b1df8335), but not with additional security patches concerning the current security issues: if the implications of that are not fully understood, people should not test these kernels on systems that have security or privacy implications! Beyond, consider the usual precautions as elaborated below.
...
```
cat kernel-7.0.10-5.fc44.ark.patched-upstream-stable.tar.xz.split* > kernel-7.0.10-5.fc44.ark.patched-upstream-stable.tar.xz
tar xJf kernel-7.0.10-5.fc44.ark.patched-upstream-stable.tar.xz
cd result
sh kernelUp.sh
```
